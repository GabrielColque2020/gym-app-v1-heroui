...
import TrainingRoutinesFilter from "@/features/role/student/training-routine/components/TrainingRoutinesFilter";
import { useTrainingRoutines } from "@/features/role/student/training-routine/hooks/useTrainingRoutines";
...
export default function TrainingRoutinesPageContent({
  ...
  const effectiveSelectedWeekId = selectedWeekExists
    ? selectedWeekId
    : (routines[0]?.id ?? "");
  ...
  return (
    <div className={"flex w-full flex-col gap-4"}>
      ...
      <div className={"w-full"}>
        ...
        >
          ...
          {isLoading ? (
            <div
              className={
                "flex items-center justify-center rounded-full border border-border bg-surface px-4 py-2 text-sm text-muted"
              }
            >
              {" "}
              Cargando semanas{" "}
      ...
      {isError ? (
        <Card
          className={"border border-danger/20 bg-surface"}
          variant={"default"}
        >
          {" "}
          <Card.Content
            className={
              "flex min-h-40 flex-col items-center justify-center gap-3 py-8 text-center"
            }
          >
            {" "}
            <p className={"text-base font-semibold text-foreground"}>
          ...
          >
            ...
            <Card.Content className={"py-10 text-center"}>
              ...
              <p className={"mt-1 text-sm text-muted"}>
                {" "}
                No encontramos rutinas para{" "}
                {String(activeMonth).padStart(2, "0")}/{activeYear}.{" "}
              </p>{" "}
            </Card.Content>{" "}
          </Card>
        ) : (
            ...
            {selectedRoutine?.routineDays.map((day) => (
              ...
              >
                ...
                <div className={"flex flex-1 flex-col gap-3"}>
                  ...
                  <Card.Footer
                    className={"mt-auto flex w-full flex-col items-end gap-3"}
                  >
                    {" "}
                    <Link
                      href={`/routine?routineDayId=${day.id}`}
