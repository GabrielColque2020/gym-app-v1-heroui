	...
	return {
		...
		exercises: sourceSession.exercises.map( ( sourceExercise ) => {
			...
			return {
				...
				name: draftExercise.variantExerciseId ? draftExercise.name : sourceExercise.name,
				...
				variantOptions: sourceExercise.variantOptions.length > 0 ? sourceExercise.variantOptions : draftExercise.variantOptions ?? [],
			};
		} ),
	};
}
...
export function useRoutineSession( { routineDayId, sourceDetail }: UseRoutineSessionOptions ) {
	...
	const activeSession = useMemo( () => mergeSessionVariants( draftSession, sourceSession ), [ draftSession, sourceSession ] );
