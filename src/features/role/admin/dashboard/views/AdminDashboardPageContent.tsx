...
import { AdminDashboardHero } from "@/features/role/admin/dashboard/components/AdminDashboardHero";
import { AdminDashboardQuickActions } from "@/features/role/admin/dashboard/components/AdminDashboardQuickActions";
import { AdminDashboardQuickStats } from "@/features/role/admin/dashboard/components/AdminDashboardQuickStats";
import { AdminDashboardStudentsTable } from "@/features/role/admin/dashboard/components/AdminDashboardStudentsTable";
import { useAdminDashboardSummary } from "@/features/role/admin/dashboard/hooks/useAdminDashboardSummary";
...
export default function AdminDashboardPageContent() {
			...
			<Card className={ "border border-border bg-surface" } variant={ "default" }>
				<Card.Content className={ "flex min-h-72 flex-col items-center justify-center gap-3 py-10 text-center" }>
					...
					<div className={ "space-y-1" }>
						<p className={ "text-base font-semibold text-foreground" }>Cargando dashboard admin</p>
	...
	if (isError || !data) {
		return (
			<Alert className={ "border border-danger/20" } status={ "danger" }>
				<Alert.Content>
					<Alert.Title>Error al cargar el dashboard admin</Alert.Title>
	...
	return (
		<div className={ "flex flex-col gap-4" }>
			...
			/>
			<AdminDashboardQuickStats items={ quickStats }/>
			<AdminDashboardQuickActions/>
			<AdminDashboardStudentsTable
				currentPeriodLabel={ data.currentPeriod.label }
