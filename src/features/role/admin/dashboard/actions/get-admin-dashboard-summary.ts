...
import {
	...
	mapAdminDashboardStudent,
	RECENT_ACTIVITY_WINDOW_DAYS,
} from "@/features/role/admin/dashboard/services/admin-dashboard-mappers";
...
export type AdminDashboardStudentSummary = ReturnType<typeof mapAdminDashboardStudent>;
...
export async function getAdminDashboardSummaryAction(): Promise<AdminDashboardSummary> {
	try {
		const session = await requireCoachSession( "consultar el dashboard administrativo" );
		const now = new Date();
		...
		const studentSummaries = students
			.map( ( student ) => mapAdminDashboardStudent( {
