...
import type { AdminDashboardStudentSummary } from "@/features/role/admin/dashboard/actions/get-admin-dashboard-summary";
...
import { AdminDashboardEmptyState } from "@/features/role/admin/dashboard/components/AdminDashboardEmptyState";
import { AdminDashboardStudentRowActions } from "@/features/role/admin/dashboard/components/AdminDashboardStudentRowActions";

type AdminDashboardStudentsTableProps = {
...
function buildRoutineStatusLabel( student: AdminDashboardStudentSummary, currentPeriodLabel: string ) {
...
export function AdminDashboardStudentsTable( {
	currentPeriodLabel,
	students,
}: AdminDashboardStudentsTableProps ) {
	...
	const columns = useMemo<DataGridColumn<AdminDashboardStudentSummary>[]>( () => [
		{
			accessorKey: "name",
			allowsSorting: true,
			cell: ( student ) => (
		...
		{
			cell: ( student ) => <AdminDashboardStudentRowActions student={ student }/>,
			header: "Acciones",
			id: "actions",
			minWidth: 220,
		},
			...
			<Card.Content className={ "space-y-4 px-5 py-4 sm:px-6" }>
				<div className={ "flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between" }>
					...
					<div className={ "w-full lg:max-w-md" }>
						<SearchField
							name={ "admin-dashboard-student-search" }
							value={ searchFilter }
							onChange={ ( value ) => {
							...
							{ pagination.paginatedItems.map( ( student ) => (
								<Card key={ student.id } className={ "border border-border bg-surface-secondary" } variant={ "default" }>
									<Card.Content className={ "space-y-3 px-4 py-4" }>
										...
										<AdminDashboardStudentRowActions student={ student }/>
									</Card.Content>
								</Card>
							) ) }
						</div>
