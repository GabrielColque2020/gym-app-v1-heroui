...
import type { TrainingRoutinesStudentListItem } from "@/features/role/admin/training-routines-students/actions/get-training-routines-students";
...
import { useTrainingRoutinesStudentsList } from "@/features/role/admin/training-routines-students/hooks/useTrainingRoutinesStudentsList";

type TrainingRoutinesStudentsContentMobileProps = {
	students: TrainingRoutinesStudentListItem[];
};

function TrainingRoutineStudentMobileCard( { student }: { student: TrainingRoutinesStudentListItem } ) {
			...
			<Card.Content className={ "py-1" }>
				<div className={ "grid grid-cols-[4rem_1fr_auto] items-start gap-3" }>
					...
					<Button
						...
						variant={ "ghost" }
						onPress={ () => router.push( `/admin/trainingRoutine?studentId=${ student.id }` ) }
					>
...
export function TrainingRoutinesStudentsContentMobile( { students }: TrainingRoutinesStudentsContentMobileProps ) {
	const {
		changePage,
		clearFilters,
		filteredStudents,
				...
				<>
					<div className={ "grid gap-3" }>
						{ paginatedStudents.map( ( student ) => (
							<TrainingRoutineStudentMobileCard key={ student.id } student={ student }/>
						) ) }
					</div>
