...
import type { MealPlansStudentListItem } from "@/features/role/admin/meal-plans-students/actions/get-meal-plans-students";
...
import { useMealPlansStudentsList } from "@/features/role/admin/meal-plans-students/hooks/useMealPlansStudentsList";

type MealPlansStudentsContentMobileProps = {
	students: MealPlansStudentListItem[];
};

function MealPlansStudentMobileCard( { student }: { student: MealPlansStudentListItem } ) {
...
export function MealPlansStudentsContentMobile( { students }: MealPlansStudentsContentMobileProps ) {
	const {
		changePage,
		clearFilters,
		filteredStudents,
				...
				<>
					<div className={ "grid gap-3" }>
						{ paginatedStudents.map( ( student ) => (
							<MealPlansStudentMobileCard key={ student.id } student={ student }/>
						) ) }
					</div>
