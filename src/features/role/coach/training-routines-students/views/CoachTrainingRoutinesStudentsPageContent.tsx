...
import { TrainingRoutinesStudentsContentDesktop } from "@/features/role/coach/training-routines-students/components/desktop/TrainingRoutinesStudentsContentDesktop";
import { TrainingRoutinesStudentsContentMobile } from "@/features/role/coach/training-routines-students/components/mobile/TrainingRoutinesStudentsContentMobile";
import { useTrainingRoutinesStudents } from "@/features/role/coach/training-routines-students/hooks/useTrainingRoutinesStudents";

// Renderiza la seleccion de estudiantes para consultar sus rutinas asignadas.
export default function CoachTrainingRoutinesStudentsPageContent() {
	const { data: students = [], error, isError, isFetching, isLoading, refetch } = useTrainingRoutinesStudents();
	const breadcrumbs = [
		{ href: "/", label: "Inicio" },
				...
				<Card className={ "border border-border bg-surface" } variant={ "default" }>
					<Card.Content className={ "flex min-h-56 flex-col items-center justify-center gap-3 py-10 text-center" }>
						...
						<div className={ "space-y-1" }>
							...
							<p className={ "text-sm text-muted" }>Consultando estudiantes disponibles para rutinas.</p>
						</div>
					</Card.Content>
				</Card>
			</>
