...
import type {
	...
	DeleteMealPlanInput,
	UpdateMealPlanInput,
} from "@/features/admin/mealPlans/services/meal-plans-form";
import { isMealTimeValue, type MealTimeValue } from "@/features/admin/mealPlans/services/meal-plans-form";
...
export async function createMealPlanAction( input: CreateMealPlanInput ) {
	try {
		...
		if (!session) {
			throw new Error( "Debes iniciar sesion para crear planes alimenticios." );
		}

		if (session.role !== "COACH") {
			throw new Error( "No tienes permisos para crear planes alimenticios." );
		}
...
export async function updateMealPlanAction( input: UpdateMealPlanInput ) {
	try {
		...
		if (!session) {
			throw new Error( "Debes iniciar sesion para editar planes alimenticios." );
		}

		if (session.role !== "COACH") {
			throw new Error( "No tienes permisos para editar planes alimenticios." );
		}
...
export async function deleteMealPlanAction( input: DeleteMealPlanInput ) {
	try {
		...
		if (!session) {
			throw new Error( "Debes iniciar sesion para eliminar planes alimenticios." );
		}

		if (session.role !== "COACH") {
			throw new Error( "No tienes permisos para eliminar planes alimenticios." );
		}
