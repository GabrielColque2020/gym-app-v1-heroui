...
import { useTrainingRoutineSelection } from "@/features/admin/trainingRoutine/hooks/useTrainingRoutineSelection";
...
type AdminTrainingRoutineCardDesktopProps = {
	routines: AdminTrainingRoutine[];
};

export function AdminTrainingRoutineCardDesktop( { routines }: AdminTrainingRoutineCardDesktopProps ) {
			...
			<Card className={ "col-span-4 w-full self-start overflow-hidden" }>
				<Card.Header className={ "gap-3 pb-3 pt-2" }>
					<Card.Content className={ "min-w-0" }>
						...
						<Chip className={ "w-fit shrink-0 px-2" } color={ "accent" } size={ "sm" } variant={ "soft" }>
					...
					<ScrollShadow className={ "max-h-105 pr-1" }>
						...
						>
							{ routines.map( ( routine ) => (
								<RadioButtonGroup.Item key={ routine.id } className={ "w-full gap-2 px-3 py-2.5" } value={ routine.id }>
			...
			<Card className={ "col-span-8 w-full overflow-hidden" }>
					...
					<div className={ "flex w-full items-center justify-between gap-3" }>
						<div className={ "flex min-w-0 flex-col gap-1" }>
							<Card.Title className={ "truncate text-base font-semibold" }>
								{ selectedRoutine ? `Semana ${ selectedRoutine.week }` : "Sin semana seleccionada" }
							</Card.Title>
				...
				<Card.Content className={ "pb-4" }>
					<AdminTrainingRoutineDaysAccordion
						...
						exerciseGridClassName={ "grid grid-cols-2 gap-2 rounded-xl bg-default/40 p-3" }
					/>
				</Card.Content>
			</Card>
		</div>
