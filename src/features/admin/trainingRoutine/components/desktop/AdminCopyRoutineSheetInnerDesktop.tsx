...
import { MONTH_OPTIONS } from "../shared/adminCopyRoutineConstants";
import {
	type AdminCopyRoutineSheetProps,
	useAdminCopyRoutineSheetState,
} from "@/features/admin/trainingRoutine/components/shared/useAdminCopyRoutineSheetState";
					...
					<div className={ "grid min-h-0 gap-3 md:flex-1 md:grid-cols-[1fr_260px] md:gap-4" }>
									...
									<>
													...
													<CheckboxButtonGroup.Item key={ routine.week } className={ "min-h-16 gap-2 px-3 py-2 sm:min-h-0 sm:py-3" } value={ String( routine.week ) }>
														...
														<CheckboxButtonGroup.ItemContent>
															...
															<Description className={ "line-clamp-1 text-xs" }>
																{ routine.dayCount } días / { routine.exerciseCount } ejercicios
															</Description>
														</CheckboxButtonGroup.ItemContent>
										...
										{ selectedSorted.length === 0 ? (
											...
											<div className={ "grid gap-3" }>
												...
												>
													{ [ "1", "2", "3", "4" ].map( ( week ) => (
														<CheckboxButtonGroup.Item key={ week } className={ "min-h-12 gap-2 px-3 py-2 sm:min-h-0 sm:py-3" } value={ week }>
															<CheckboxButtonGroup.Indicator/>
															<CheckboxButtonGroup.ItemContent>
						...
						<Surface className={ "flex min-h-0 flex-col gap-2 rounded-xl border border-default-hover bg-surface p-3 sm:gap-3 sm:p-4" }>
							...
							<div className={ "grid gap-1.5 sm:gap-2" }>
								...
								<SummaryRow label={ "Días origen" } value={ selectedSourceRoutineStats.dayCount || "-" }/>
