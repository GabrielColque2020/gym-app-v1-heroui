...
function AddExercisePickerSheetContent( {
											addedExerciseIds,
											onAddExercise,
											placement,
											suggestedOrder,
	...
	const sheetContentClassName = placement === "right" ? "w-105" : "w-full max-h-[85vh]";

	return (
		<>
			<Sheet isDetached isOpen={ isPickerOpen } placement={ placement } onOpenChange={ setIsPickerOpen }>
				<Sheet.Trigger>
					<AddExercisePickerButton onPress={ () => setOrderValue( String( suggestedOrder ) ) }/>
				</Sheet.Trigger>
				<Sheet.Backdrop variant={ "opaque" }>
					<Sheet.Content className={ sheetContentClassName }>
						<Sheet.Dialog className={ placement === "right" ? "flex h-full max-h-screen min-h-0 flex-col" : "flex min-h-0 flex-col" }>
